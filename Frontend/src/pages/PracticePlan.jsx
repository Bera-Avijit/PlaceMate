...
import { ArrowLeft, BookOpen, Lightbulb, Clock, CheckCircle2, Layout, AlertTriangle } from 'lucide-react';
import Navbar from '../components/Navbar';
import { useAuth } from '../context/AuthContext';
import { generateQuestions } from '../services/resumeService';
...
const PracticePlan = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      try {
        const result = await generateQuestions(user.uid, companyName, dayNumber);
        if (result.success && result.data && result.data.length > 0) {
          // Sort by session number just in case
      ...
      <main className="max-w-4xl mx-auto px-6 pt-32 pb-24">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 bg-[#0a0a0a] border border-white/5 rounded-[3rem] shadow-2xl relative overflow-hidden">
             ...
             <h2 className="text-2xl font-black uppercase tracking-widest mb-2">Generating Questions</h2>
             <p className="text-slate-500 text-xs font-bold uppercase tracking-widest animate-pulse">Running AI generation pipeline...</p>
          </div>
        ) : error ? (
          <div className="bg-[#0a0a0a] border border-red-500/20 rounded-[3rem] p-16 text-center shadow-2xl relative overflow-hidden">
             ...
             <button onClick={() => navigate(`/plan/${companyName}`)} className="px-8 py-3 bg-white/5 hover:bg-white/10 rounded-xl font-black text-[10px] uppercase tracking-widest transition-colors">
               Return to Prep Plan
             </button>
          </div>
        ) : questions.length > 0 && (
          ...
          >
            <div className="mb-12 border-b border-white/5 pb-8 relative">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-slate-400 text-xs font-bold uppercase tracking-widest flex items-center gap-2 bg-white/5 px-4 py-1.5 rounded-full border border-white/5">
                  <Clock size={12} className="text-amber-500" /> {questions[0].estimated_total_time || "N/A"}
                </span>
              </div>
              ...
              <p className="text-slate-400 text-base font-medium max-w-2xl">
                Complete these practice questions to solidify your understanding before moving on.
              </p>
            </div>

            <div className="space-y-8">
              {questions.map((q, idx) => (
                <div key={q.id || idx} className="bg-[#0a0a0a] border border-white/5 rounded-[2rem] p-8 md:p-10 hover:border-amber-500/20 transition-colors shadow-2xl relative overflow-hidden group">
                   ...
                   <div className="flex flex-wrap items-center gap-3 mb-6">
                     ...
                     <span className={`px-3 py-1 border font-black text-[10px] uppercase tracking-widest rounded-lg ${
                   ...
                   {q.hint && (
                     <div className="flex items-start gap-3 bg-blue-500/5 border border-blue-500/10 rounded-xl p-5 mb-6">
                       ...
                       <div>
                         <h4 className="text-[10px] font-black text-blue-500 uppercase tracking-widest mb-1">Hint</h4>
                         <p className="text-sm text-slate-300 font-medium">{q.hint}</p>
                       </div>
                     </div>
                   )}
                   ...
                   <div className="flex flex-wrap items-center gap-4 justify-between pt-6 border-t border-white/5">
                     <div className="flex items-center gap-2">
                       {q.resource_url && (
                         <a href={q.resource_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/5 rounded-lg text-[10px] font-black text-slate-300 uppercase tracking-wider transition-colors">
            ...
            <div className="mt-16 text-center">
               <button onClick={() => navigate(`/plan/${companyName}`)} className="px-8 py-3 bg-amber-500 hover:bg-amber-400 text-black rounded-xl font-black text-[10px] uppercase tracking-widest transition-colors shadow-lg shadow-amber-500/20">
